...
class AIService {
     ...
     */
    async saveMemoryWithAI(content, privacyLevel = 'personal', categoryHint) {
        try {
...
`;
     ...
     */
    async searchMemoriesOnly(question, maxPrivacy = 'personal', limit = 5) {
        try {
     ...
     */
    async queryMemoriesWithAI(question, maxPrivacy = 'personal', limit = 5) {
        try {
...
`;
        ...
        try {
            ...
            for (const level of privacyLevels) {
                try {
                    ...
                    status.memories[level] = count;
                    status.memories.total += count;
                }
                catch (e) {
                    status.memories[level] = 0;
                }
            }
        }
        catch (error) {
     ...
     */
    async searchMemories(query, maxPrivacy, limit) {
