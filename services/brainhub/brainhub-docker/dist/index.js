    ...
    setupInstanceRoutes() {
        ...
        this.app.get('/api/instances/:name/logs/stream', async (req, res) => {
            ...
            dockerLogs.stdout.on('data', (data) => {
                ...
                res.write(`data: ${JSON.stringify({ instance: name, log })}\n\n`);
            });
    ...
    setupWebSocket() {
        this.io.on('connection', (socket) => {
            ...
            socket.on('subscribe_logs', (instanceName) => {
                ...
                dockerLogs.stdout.on('data', (data) => {
                    socket.emit('brain_log', { instance: instanceName, log: data.toString().trim() });
                });
                dockerLogs.stderr.on('data', (data) => {
                    socket.emit('brain_log', { instance: instanceName, log: data.toString().trim() });
                });
                ...
                case 'message_received':
                    ...
                    if (flowInfo) {
                        if (flowInfo.to === 'brainxchange' || flowInfo.from === 'brainxchange') {
                            this.io.to('bx_network').emit('bx_network_log', `🌐 ${event.type}: ${flowInfo.from} → ${flowInfo.to} via wss://m3u.dossant.com`);
                        }
                        else if (flowInfo.to !== flowInfo.from) {
                            this.io.to('bx_network').emit('bx_network_log', `🧠 Cross-brain: ${flowInfo.from} → ${flowInfo.to}`);
                        }
                    }
                    break;
            }
