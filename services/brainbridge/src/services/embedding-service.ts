...
export class EmbeddingService {
  ...
  async processMemoryFile(filePath: string): Promise<MemoryEmbedding> {
    ...
    return {
      ...
      filePath,
      content: mainContent,
      contentPreview: this.generateContentPreview(mainContent, metadata.title),
      embedding,
      metadata
    };
  }
  ...
  /**
   * Fast search using index data only (no file I/O)
   */
  async searchSimilarFast(query: string, limit: number = 5, threshold: number = 0.5): Promise<Array<{
    id: string;
    title: string;
    contentPreview: string;
    content: string;
    ...
    this.loggerService.trace('Starting fast vector search (index-only)', { query, limit, threshold });
    this.loggerService.startTimer('fast_vector_search');
  ...
  private generateContentPreview(content: string, title?: string): string {
    // Remove markdown syntax and excessive whitespace
    ...
    // Add title context if available and different
    if (title && !preview.toLowerCase().includes(title.toLowerCase().substring(0, 10))) {
      return `${title}: ${preview}`;
    }
    
    return preview || title || '[No preview available]';
  }
}